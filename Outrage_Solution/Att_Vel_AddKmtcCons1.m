function res = Att_Vel_AddKmtcCons1(imu, mavp, imuerr, davp,avp0,index)
%% -----------Introduction------------
%姿态匹配+速度匹配+ 车体运动学速度匹配 23维传递对准算法 估计主子安装角，车体安装角，杆臂
%input: 
...
kf = []; m  = 8; n =23;
kf.m = m;  kf.n = n;
kf.Pk = 10*diag([davp(1:3); davp(4:6);davp(7:9);imuerr.eb; imuerr.db;[10;10;10]*pi/180;[10;10;10];[10;10]*pi/180])^2;
...
for i = 2:N    
    ...
    if ftrsn == 1        
        ...
        kf.Hk = [eye(3)  zeros(3,3) zeros(3,9) -Cnb     zeros(3,5); 
                 zeros(3,3) eye(3)  zeros(3,9)  zeros(3,3) Cnb*askew(web)  zeros(3,2);
                  M2(1,:) M1(1,:)    zeros(1,15) 0       M3(1,3);
                  M2(3,:) M1(3,:)    zeros(1,15) M3(3,1) 0;
                 ];
        kf.Rk = diag([[100; 100; 100;].*pi/180/60;[0.1;0.1;0.1];[0.5;0.5]])^2;                      %[0.5;0.5] 姿态速度匹配  
        Zk = [0.5*(z(3,2)-z(2,3)); 
              0.5*(z(1,3)-z(3,1)); 
              0.5*(z(2,1)-z(1,2));
              vel-vnm;
        ...
        M2 = -Cmb*Cbn*askew(vel);M1 = Cmb*Cbn;M3 = askew(vm); 
        kf.Hk = [M2(1,:)  M1(1,:) zeros(1,15) 0       M3(1,3);
                 M2(3,:)  M1(3,:) zeros(1,15) M3(3,1) 0;]; 
    ...
    kf.Xk(1:3) = (1-factor)*kf.Xk(1:3);  %清0
    att = q2att(qua);
    Cnb = q2mat(qua);
    % 速度
    vel = vel-factor*kf.Xk(4:6);  
    kf.Xk(4:6) = (1-factor)*kf.Xk(4:6);  %清0
    %位置
    if ftrsn == 1  
        pos = mavp(i,7:9)';
        kf.Xk(7:9)  = zeros(3,1);
